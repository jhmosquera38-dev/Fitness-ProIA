...
import { coachService, CoachService } from '../services/coachService';
...
const StatCard: React.FC<{ label: string; value: string | number; icon: string; colorClass?: string }> = ({ label, value, icon, colorClass = "text-slate-800 dark:text-white" }) => (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4 transition-transform hover:scale-105">
        <div className="w-12 h-12 rounded-full bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-2xl shadow-inner">
            {icon}
        </div>
        <div>
...
// Componente de Tarjeta de Servicio (Clases ofrecidas)
const ServiceCard: React.FC<{ service: CoachService }> = ({ service }) => (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-3">
            ...
            <span className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-xs font-bold px-2 py-1 rounded-lg">
                ${service.price}
            </span>
        </div>
        <p className="text-slate-500 dark:text-slate-400 text-xs mb-4 line-clamp-2">{service.description}</p>
        <div className="flex justify-between items-center text-xs text-slate-400 dark:text-slate-500">
            <span>👥 {service.activeClients || 0} Activos</span>
            <span>⭐ 4.9</span>
        </div>
    </div>
);
...
export const CoachDashboard: React.FC<CoachDashboardProps> = ({ onNavigate }) => {
    ...
    const [myClasses, setMyClasses] = useState<any[]>([]); // New State for Classes
    const [clients, setClients] = useState<any[]>([]);
    const [reviews, setReviews] = useState<any[]>([]);
    const [stats, setStats] = useState({ income: 0, rating: 0, clients: 0 });
    const [isLoading, setIsLoading] = useState(true);
    const { showToast } = useFeedback();

    // Modals
    const [classModalOpen, setClassModalOpen] = useState(false);
    const [newClass, setNewClass] = useState({ name: '', description: '', coach: '', date: '', time: '', capacity: 20, category: 'Funcional' });
    ...
    useEffect(() => {
        const loadDashboard = async () => {
            try {
                setIsLoading(true);
                const [statsData, myServices, myClassesData, clientsData, reviewsData] = await Promise.all([
                    coachService.getStats(),
                    coachService.fetchMyServices(),
                    groupClassService.fetchMyClasses(),
                    coachService.fetchMyClients(),
                    coachService.fetchReviews()
                ]);
    ...
    const handleSaveClass = async () => {
        try {
            const classData = {
                ...
                // Preserve other fields if needed or set defaults
            };
            ...
            if (editingClassId) {
                ...
                // Create
                await groupClassService.createClass({
                    ...classData,
            ...
            // Refresh
            const updatedClasses = await groupClassService.fetchMyClasses();
            setMyClasses(updatedClasses);
            setClassModalOpen(false);
            setEditingClassId(null);
            setNewClass({ name: '', description: '', coach: '', date: '', time: '', capacity: 20, category: 'Funcional' }); // Reset
        ...
        <div className="container mx-auto px-4 py-8 md:py-10 space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-slate-200 dark:border-slate-700 pb-6">
                ...
                <div className="flex gap-3">
                    <button onClick={() => { setEditingClassId(null); setNewClass({ name: '', description: '', coach: '', date: '', time: '', capacity: 20, category: 'Funcional' }); setClassModalOpen(true); }} className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-white font-semibold py-2 px-4 rounded-lg hover:bg-slate-200 transition-colors text-sm">
                        + Publicar Clase
                    </button>
                </div>
            </div>
